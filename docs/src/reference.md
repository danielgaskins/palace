...
Otherwise, Palace searches the version-3 library's explicitly qualified
`CornerRadiusInterpolation` spans. It linearly combines the endpoints only when a
held-out coupon has qualified that span, both endpoint radii are positive, and topology,
angle, and boundary law are compatible. A sharp-corner model is not an interpolation
endpoint, and Palace does not extrapolate outside a qualified range. A rounded corner
with neither an exact match nor a qualified span remains uncorrected and contributes to
the unmatched-corner diagnostics. `UnmatchedPolicy: "Error"` rejects such a corner;
`Warn` retains the documented straight-edge fallback. The response operators and
conductor reference are combined with convex weights; the normalized bracket width
contributes to `max library distance`. Locally connected arms are not treated as an
unsupported nearby-edge pair. An exact
...
Rounded-radius interpolation is opt-in at the top level of a version-3 process library:

```json
{
  "CornerRadiusInterpolation": [
    {
      "LowerModel": "convex-corner-r0.25",
      "UpperModel": "convex-corner-r0.75",
      "Qualification": {
...
`LowerModel` and `UpperModel` must name distinct compatible corner models whose positive
radii strictly bracket `HeldoutRadius`. Palace does not infer permission to interpolate
from adjacent models alone. The example
`corner_coupon/qualify_corner_interpolation.py` compares an independent held-out coupon
against the weighted endpoint domain, surface, defect, fixed-trace, and fixed-flux
responses. `combine_process_libraries.py --corner-interpolation-qualification REPORT`
embeds only a passed report whose endpoint models are present in the combined library.
...
candidates additionally reject nonpositive high-order Jacobians. A manifold trace
